...
//Test github commit
public class VisionPipeline_BLUE_SCUFF extends OpenCvPipeline
{
    Telemetry tel;
    public VisionPipeline_BLUE_SCUFF(Telemetry tel)
    {
        this.tel=tel;
    }
    ...
    public Mat processFrame(Mat input)
    {
        ...
        for(int i = 0; i < REGION_HEIGHT; i++) {
            for (int j = 0; j < REGION_WIDTH; j++) {
                counter1 += region1_Blue.get(i, j)[2];
                counter2 += region2_Blue.get(i, j)[2];
                counter3 += region3_Blue.get(i, j)[2];
